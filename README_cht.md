...
- `dist/capsousb_test.exe`：預先 build 好的 Win32 debug sample executable。
...
預先 build 好的 sample executable 放在 `dist/capsousb_test.exe`。若 sample source
有變更，請從 `src_new/exe/capsousb_test.vcxproj` 重新 build 並更新此檔。
...
5. 執行 `dist/capsousb_test.exe` 或本機重新 build 的 sample application。
