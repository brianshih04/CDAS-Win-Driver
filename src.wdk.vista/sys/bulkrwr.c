...
#ifdef CDAS2_CHANGES
            else if(uval >= 6 && pipeContext != NULL && nameLength == 0) {
                pipeContext = &deviceExtension->PipeContext[uval-6];
				BulkUsb_DbgPrint(3, ("BulkUsb_PipeWithName - ix %ld, uval %ld\n", ix, uval));
            }
#endif
        }
    }
...
#ifdef CDAS2_CHANGES
    BulkUsb_DbgPrint(3, ("totalLength is %ld\n", totalLength));
#endif
