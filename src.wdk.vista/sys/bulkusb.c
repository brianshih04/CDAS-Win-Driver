...
--*/
{
...
#ifdef CDAS2_CHANGES
	PUNICODE_STRING devGUID;
#endif    
    PAGED_CODE();

    BulkUsb_DbgPrint(3, ("DriverEntry\n"));
    //
...
#ifdef CDAS2_CHANGES
	devGUID = &Globals.BulkUsb_ClassDevGUID;
#endif
    //
    ...
    BulkUsb_DbgPrint(3, ("Allocate memory success for CapsoVision registryPath\n"));
...
#ifdef CDAS2_CHANGES
	// Initialize CapsoVision class device GUID
    devGUID->MaximumLength = UniRegistryPath->Length + sizeof(UNICODE_NULL);
    devGUID->Length        = UniRegistryPath->Length;
    devGUID->Buffer        = ExAllocatePool(PagedPool, registryPath->MaximumLength);

    if (!devGUID->Buffer) {

        BulkUsb_DbgPrint(1, ("Failed to allocate memory for CapsoVision class devGUID\n"));
        ntStatus = STATUS_INSUFFICIENT_RESOURCES;
        goto DriverEntry_Exit;
    } 
    BulkUsb_DbgPrint(3, ("Allocate memory success for CapsoVision class devGUID\n"));

	RtlZeroMemory (devGUID->Buffer, 
                   devGUID->MaximumLength);
    RtlMoveMemory (devGUID->Buffer, 
                   UniRegistryPath->Buffer, 
                   UniRegistryPath->Length);
#endif
...
--*/
{
    ...
    if(devGUID->Buffer) {
        ...
        ExFreePool(devGUID->Buffer);
        devGUID->Buffer = NULL;
    }
	BulkUsb_DbgPrint(3, ("BulkUsb_DriverUnload - devGUID released\n"));
#endif
	if(registryPath->Buffer) {
    ...
    BulkUsb_DbgPrint(3, ("BulkUsb_DriverUnload - registryPath released\n"));
