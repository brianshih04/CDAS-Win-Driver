...
#ifdef CDAS2_CHANGES
#define WMI_BULKUSB_CDAS1_INFORMATION 1
#define WMI_BULKUSB_CDAS2_INFORMATION 2
#endif
...
#ifdef CDAS2_CHANGES
    WCHAR             modelCDAS1Name[] = L"AishveryaCDAS1\0\0";
    WCHAR             modelCDAS2Name[] = L"AishveryaCDAS2\0\0";
#endif
	USHORT            modelNameLen;
...
#ifdef CDAS2_CHANGES
	case WMI_BULKUSB_CDAS1_INFORMATION:
	    modelNameLen = (USHORT)((wcslen(modelCDAS1Name) + 1) * sizeof(WCHAR));
        size = sizeof(ULONG) + modelNameLen + sizeof(USHORT);

        if (OutBufferSize < size ) {
            BulkUsb_DbgPrint(3, ("CDAS1 OutBuffer too small\n"));
            ntStatus = STATUS_BUFFER_TOO_SMALL;
            break;
        }
        ...
        * (PULONG) Buffer = DebugLevel;
        ...
        //
        // put CDAS1 length of string ahead of string
        //

        *((PUSHORT)Buffer) = modelNameLen;
        Buffer = (PUCHAR)Buffer + sizeof(USHORT);
        RtlCopyBytes((PVOID)Buffer, (PVOID)modelCDAS1Name, modelNameLen);
        *InstanceLengthArray = size ;
        ntStatus = STATUS_WMI_GUID_NOT_FOUND;
        break;

	case WMI_BULKUSB_CDAS2_INFORMATION:
	    modelNameLen = (USHORT)((wcslen(modelCDAS2Name) + 1) * sizeof(WCHAR));
        size = sizeof(ULONG) + modelNameLen + sizeof(USHORT);

        if (OutBufferSize < size ) {
            BulkUsb_DbgPrint(3, ("CDAS2 OutBuffer too small\n"));
            ntStatus = STATUS_BUFFER_TOO_SMALL;
            break;
        }
        ...
        * (PULONG) Buffer = DebugLevel;
        ...
        //
        // put CDAS2 length of string ahead of string
        //

        *((PUSHORT)Buffer) = modelNameLen;
        Buffer = (PUCHAR)Buffer + sizeof(USHORT);
        RtlCopyBytes((PVOID)Buffer, (PVOID)modelCDAS2Name, modelNameLen);
        *InstanceLengthArray = size ;
        ntStatus = STATUS_WMI_GUID_NOT_FOUND;
        break;
#endif
